# Getting Started with bot_army_decomp_coach

This guide walks you through setting up Decomposition Coach Bot for local development.
...
DATABASE_NAME=bot_army_decomp_coach_dev
```
...
Decomposition Coach Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_decomp_coach/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_decomp_coach/skills/example.ex lib/bot_army_decomp_coach/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyDecompCoach.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_decomp_coach.command.classify"]
...
```elixir
# In lib/bot_army_decomp_coach/application.ex or similar

defmodule BotArmyDecompCoach.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyDecompCoach.Skills.Classify,
      BotArmyDecompCoach.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_decomp_coach/skills/my_skill.ex
...
releases: [
  decomp_coach_bot: [
    applications: [bot_army_decomp_coach: :permanent]
  ]
]
```

This creates a release named `decomp_coach_bot` that is deployed to `/opt/ergon/releases/decomp_coach_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/decomp_coach_bot/`
