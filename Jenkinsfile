pipeline {
  ...
  environment {
    BOT_NAME = 'decomp_coach_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-decomp_coach"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/decomp_coach_bot/bin/decomp_coach_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyDecompCoach.Release.create()'"
          $RELEASE_BIN eval 'BotArmyDecompCoach.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyDecompCoach.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyDecompCoach.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_decomp_coach/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_decomp_coach/releases/start_erl.data)
        fi
